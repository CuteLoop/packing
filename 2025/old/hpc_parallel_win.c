...
/* forward declaration so maybe_stop_and_flush can reference g_logger before its definition */
typedef struct { FILE *telemetry; FILE *snapstats; char telemetry_path[256]; char snapstats_path[256]; int have_header; } Logger_fwd;
static Logger_fwd *g_logger;
...
typedef Logger_fwd Logger;

/* g_logger already declared above as Logger_fwd* — reuse it as Logger* (same layout) */
