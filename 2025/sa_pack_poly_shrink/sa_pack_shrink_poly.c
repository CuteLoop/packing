...
//   (A) Performance boost: per-instance TRIANGLE AABB cache + triangle-AABB reject
//       before SAT => far fewer SAT calls.
//   (B) Bracketing fix: L_low is guaranteed truly infeasible via an AREA lower bound.
//       Specifically: if L^2 < N * area(poly), packing is impossible.
//       We enforce L_low <= (1 - eps)*sqrt(N*area) so it is mathematically infeasible.
//   (C) High-ROI refinement: deterministic "best grid shape" initializer that picks
//       cols/rows to minimize max(width,height) based on a fixed-theta AABB cell.
//   (D) Output hygiene: SVGs go to img/, CSVs go to csv/ (directories auto-created).
//
// Defaults (as requested):
//   - theta in [0, 2π)
//   - mixed moves: 70% XY jiggle, 30% (XY + rotation)
//   - broad-phase: AABB + bounding circle
//
// High-ROI upgrades included:
// 1) Incremental energy updates: neighbor-only via uniform grid (spatial hash)
// 2) Adaptive step size: windowed acceptance-rate controller (step_xy and step_th)
// 3) Two-phase schedule: Explore -> Enforce feasibility (penalty ramp in phase B)
// 4) Multi-start: many medium trials beats one long run
// 5) Reinsert move (teleport): cheap global escape
// 6) OUTER LOOP: shrink L via bracket + binary search
//
// Outputs:
//   - csv/best_polys.csv   (i,cx,cy,theta_rad)
//   - img/best.svg         (renders polygons)
//
// Compile:
//   gcc -O2 -std=c11 -Wall -Wextra -pedantic sa_pack_shrink_poly.c -o sa_pack_shrink_poly -lm
//
// Run:
//   ./sa_pack_shrink_poly
//
...
static void ensure_dir(const char *path) {
    if (mkdir(path, 0755) != 0) {
        if (errno != EEXIST) {
            fprintf(stderr, "ERROR: cannot create dir '%s' (errno=%d)\n", path, errno);
            exit(1);
        }
    }
}
...
// Shoelace polygon area (absolute). Assumes BASE_V are in boundary order (as in your definition).
...
static inline AABB aabb_of_tri_pts(const Vec2 p0, const Vec2 p1, const Vec2 p2) {
    ...
    if (p1.x < b.minx) b.minx = p1.x; if (p1.x > b.maxx) b.maxx = p1.x;
    if (p1.y < b.miny) b.miny = p1.y; if (p1.y > b.maxy) b.maxy = p1.y;

    if (p2.x < b.minx) b.minx = p2.x; if (p2.x > b.maxx) b.maxx = p2.x;
    if (p2.y < b.miny) b.miny = p2.y; if (p2.y > b.maxy) b.maxy = p2.y;
...
typedef struct {
    ...
    double br;       // bounding radius of base polygon

    Grid grid;       // neighbor queries
    double cell;     // cached grid cell size
} State;
...
static State state_alloc(int N) {
    ...
    // Recommended default: cell = 2*br (simple and robust)
    s.cell = 2.0 * s.br;
    if (s.cell < 1e-9) s.cell = 1e-9;

    // init grid with current s.L (caller will set s.L later and rebuild)
...
static void update_instance(State *s, int i) {
    ...
    // Triangle AABB cache for triangle-level pruning
...
// ---------------- Initialization + warm-start scaling ----------------
...
// ---------------- High-ROI grid initializer (deterministic UB) ----------------

static AABB base_aabb_rot(double theta) {
    Vec2 tmp[NV];
    double c = cos(theta), ss = sin(theta);
    for (int i = 0; i < NV; i++) {
        tmp[i].x = c*BASE_V[i].x - ss*BASE_V[i].y;
        tmp[i].y = ss*BASE_V[i].x + c*BASE_V[i].y;
    }
    return aabb_of_verts(tmp);
}

static double state_tight_square_L_from_aabbs(const State *s) {
    AABB b = s->aabb[0];
    for (int i = 1; i < s->N; i++) {
        if (s->aabb[i].minx < b.minx) b.minx = s->aabb[i].minx;
        if (s->aabb[i].miny < b.miny) b.miny = s->aabb[i].miny;
        if (s->aabb[i].maxx > b.maxx) b.maxx = s->aabb[i].maxx;
        if (s->aabb[i].maxy > b.maxy) b.maxy = s->aabb[i].maxy;
    }
    double W = b.maxx - b.minx;
    double H = b.maxy - b.miny;
    double L = (W > H) ? W : H;
    // tiny pad for numerical stability
    return L * (1.0 + 1e-12);
}

static void state_recenter_to_origin(State *s) {
    // Center the combined AABB at the origin.
    AABB b = s->aabb[0];
    for (int i = 1; i < s->N; i++) {
        if (s->aabb[i].minx < b.minx) b.minx = s->aabb[i].minx;
        if (s->aabb[i].miny < b.miny) b.miny = s->aabb[i].miny;
    ...
    double cx0 = 0.5 * (b.minx + b.maxx);
    double cy0 = 0.5 * (b.miny + b.maxy);
    for (int i = 0; i < s->N; i++) {
        s->cx[i] -= cx0;
        s->cy[i] -= cy0;
    }
...
static void best_grid_shape(int N, double w, double h, double gap,
                            int *best_cols, int *best_rows, double *best_L)
{
    double sx = w + gap;
    double sy = h + gap;
    if (sx <= 0.0) sx = 1e-9;
    ...
    for (int cols = 1; cols <= N; cols++) {
        ...
        double W = (cols - 1) * sx + w;
        double H = (rows - 1) * sy + h;
        double L = (W > H) ? W : H;
        if (L < bL) { bL = L; bc = cols; br = rows; }
    }
...
static double init_compact_grid_best(State *s,
                                     int snake_order,
                                     double theta_fixed,
                                     double gap,
                                     int *out_cols,
                                     int *out_rows)
{
    ...
    AABB ba = base_aabb_rot(theta_fixed);
    double w = (ba.maxx - ba.minx);
    double h = (ba.maxy - ba.miny);

    int cols = 1, rows = N;
    double L_est = 0.0;
    best_grid_shape(N, w, h, gap, &cols, &rows, &L_est);
    ...
    for (int r = 0; r < rows && idx < N; r++) {
        ...
        if (snake_order && (r & 1)) { c_start = cols - 1; c_end = -1; c_step = -1; }

        for (int c = c_start; c != c_end && idx < N; c += c_step) {
            s->cx[idx] = (double)c * sx;
            s->cy[idx] = (double)r * sy;
            s->th[idx] = wrap_angle_0_2pi(theta_fixed);
            idx++;
        }
    }

    update_all(s);
    state_recenter_to_origin(s);
    update_all(s);

    double L_tight = state_tight_square_L_from_aabbs(s);
    s->L = L_tight;
    rebuild_grid(s);
                         ...
                         double p_reinsert, double p_rotmix)
{
    ...
    // old local contributions (neighbor-only)
    ...
    // update cached geometry and grid cell for k
    update_instance(s, k);
    grid_update(&s->grid, k, s->cx[k], s->cy[k]);

    // new local contributions (neighbor-only)
...
static inline int guaranteed_infeasible_by_area(double L, double area_lb) {
    // If L^2 < total_area, it's impossible. We treat L <= area_lb as infeasible.
    return (L <= area_lb);
}
...
int main(void) {
    ...
    // ---- Problem size ----
    int N = 5;               // set to contest N when ready
    ...
    const double area_lb_infeas = area_lb * (1.0 - 1e-6);        // strict: guaranteed infeasible
    printf("Area bound: area(poly)=%.12g  sqrt(N*area)=%.12g  (use L_low<=%.12g as provably infeasible)\n",
           poly_area, area_lb, area_lb_infeas);

    // ---- High-ROI constructive UB initializer (fixed theta) ----
    // This produces a tight, deterministic starting configuration and L based on AABB cells.
    // Note: since SA allows rotations, this is a *starting point*, not a proof of feasibility under all rotations.
    {
        int ub_cols = 0, ub_rows = 0;
        double theta0 = 0.0;
        double gap0 = 0.0;
        int snake = 1;
        double L_grid = init_compact_grid_best(&s, snake, theta0, gap0, &ub_cols, &ub_rows);

        // Make sure we start safely above the area LB and give some headroom for early exploration.
        double L0 = L_grid * 1.15;
        if (L0 < area_lb * 1.05) L0 = area_lb * 1.05;

        // Scale the current grid placement into this slightly larger square.
        double oldL = s.L;
        s.L = L0;
        scale_positions_for_new_L(&s, oldL, s.L, 0.995);

        printf("Grid init: cols=%d rows=%d L_grid=%.12g -> start L=%.12g\n", ub_cols, ub_rows, L_grid, s.L);
    }

    // ---- Two-phase schedule (defaults) ----
    PhaseParams A;
    A.iters = 70000;
    A.T_start = 0.30;
    A.T_end   = 3e-4;
    ...
    const int trials_bracket = 10;
    const int trials_bisect  = 7;
    ...
    // Enforce area impossibility as "infeasible" regardless of SA result.
    ...
    if (feas0_flag) {
        ...
        for (int k = 0; k < bracket_max_steps; k++) {
            ...
            // If we already crossed below the provable infeasible bound, stop.
            ...
            if (feasible_flag) {
                ...
                // This L is empirically infeasible; we still prefer a provable L_low, but this is acceptable.
                L_low = s.L;

                // Ensure true infeasible: if L_low is above the provable bound, force L_low to provable bound.
        ...
        if (!found_infeas) {
            // Fallback: set to provable infeasible
            L_low = area_lb_infeas;
            printf("\nWARNING: did not find infeasible by SA shrinking; using provably infeasible L_low=%.12g\n", L_low);
        }

    } else {
        // If initial is infeasible, we still set a provable L_low based on area.
        L_low = area_lb_infeas;
        L_curr = s.L;
        ...
        for (int k = 0; k < bracket_max_steps; k++) {
            ...
            // also ensure we grow above the area bound quickly
    ...
    // Make sure bracket ordering is sane
    if (L_low >= L_high) {
        ...
        // Ensure L_low is provably infeasible
        L_low = area_lb_infeas;
    }
    ...
    for (int it = 0; it < bisect_steps; it++) {
        ...
        // If midpoint dips below area lower bound, it is guaranteed infeasible: skip SA work.
    ...
    if (!write_polys_csv("csv/best_polys.csv", &s, final_feas)) {
        fprintf(stderr, "Failed to write csv/best_polys.csv\n");
    } else {
        printf("Wrote best configuration to csv/best_polys.csv\n");
    }

    if (!write_best_svg("img/best.svg", &s, final_feas, 1100, 1100, 40.0)) {
        fprintf(stderr, "Failed to write img/best.svg\n");
    } else {
        printf("Wrote visualization to img/best.svg\n");
    }
