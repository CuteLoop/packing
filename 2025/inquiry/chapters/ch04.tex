...
\[
	heta_t \leftarrow \theta_{t-1} - \eta \lambda \theta_{t-1}
\]
\emph{in addition to} the Adam step above. (Decoupled decay is preferred over adding $\lambda \theta$ to gradients.)
...
adam_t t\theta[d];
adam_t g[d];
for (int i = 0; i < d; ++i) { t\theta[i] = (adam_t)0; g[i] = (adam_t)(i - 3); }
...
\item accidental aliasing between exttt{t\theta} and exttt{grad} buffers.
\end{itemize}
