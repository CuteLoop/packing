...
\author{%
  ...
  \thanks{Department of Mathematics, University of Arizona.
    \texttt{joelmaldonado@arizona.edu}.
    Supported by allocation \texttt{math589} on the
...
The
simpler case of unit disks packing decision problem is
NP-complete. If the decision version of a problem is NP-complete,
then its optimization version is NP-hard.
...
non-trivial geometric machinery. 
The level of implementation complexity of the NFP approach
is a risk to the fast prototyping needed for a competition. Thus
we opt for a method both simpler to implement and flexible 
but with the drawback of providing only approximate solutions.

We therefore chose to implement a penalty-based Simulated Annealing (SA) approach.
...
For this quick prototyping study aimed at a competition,
 simulated annealing by itself is the right starting point. As Sokal
...
works across a broad class of problems and is often a good reliable baseline.
For us, simulated annealing is not a last resort so much as a deliberate foundation: it
...
performs a move by selecting a random copy $i$ and proposing a new state
...
bisection, an adaptive polish phase continues to attempt smaller $L$ values until the wall time budget is exhausted,
 with the best known $L^*$ updated on every improvement.
 Restricting the search to strictly non-overlapping
configurations at all times confines it to disconnected feasible
islands in $\mathbb{R}^{2N} \times [0, 2\pi)^N$.To overcome this 
problem we allow a budget for some overlap, penalizing it continuously in the energy function,
This then guides our choice of a continuous collision metric 
instead of a binary collision test.
...
the energy change, making it the dominant runtime cost. Three collision detection
mechanisms are employed: bounding circle checks,
...
Offline, the polygon is triangulated as a fan from vertex 0
 into $T = 13$ triangles. These are grouped into three clusters
  ...
  During narrow-phase evaluation, SAT projects triangle pairs onto
          ...
          energy landscape with no gradient. By contrast,
           the depth $d$ supplies a clear directional signal.
            Then, feeding $d^2$ into the energy function generates
             the smooth guidance necessary to steer the simulated annealing 
             optimization toward feasibility from arbitrary
              starting states.



\subsection{Energy Function and Simulated Annealing Schedule}
...
The total energy of a configuration at container side length
 ...
 boundary violation of copy $i$, and $(\lambda, \mu)$ are the respective penalty weights. A configuration
  ...
  executes two sequential phases with geometrically decreasing temperature
   $$T_k = T_0\,\beta^k, \text{ where } \beta = (T_{\mathrm{end}}/T_0)^{1/M}.$$
   Phase A (Exploration; 70{,}000 iterations): 
...
\caption{Two-phase simulated annealing schedule parameters.}
\label{tab:sa}
\begin{tabular}{lcc}
\toprule
...
Feasibility Threshold $\mathcal{F}$   & \multicolumn{2}{c}{$10^{-6}$} \\
\bottomrule
\end{tabular}
\end{table}
\subsection{Outer Loop: Bisection and Polish}

The Simulated Annealing solver serves as a feasibility oracle: for a fixed
container side length $L$, it answers whether a strictly feasible packing can be found.
Because simulated annealing does not directly minimize $L$, we wrap it in an outer bisection loop.

In the 2025 implementation, bracketing does not start from a fixed
$[L_{\mathrm{area}},\,\gamma L_{\mathrm{area}}]$ interval. Instead, the run starts from a
grid-based initialization and performs shrink/grow passes to produce a valid
bracket with $L_{\mathrm{low}}$ infeasible and $L_{\mathrm{high}}$ feasible. Values at or
below $L_{\mathrm{area\_infeas}} = \sqrt{N A_p}(1-10^{-9})$ are treated as provably infeasible.

After bracketing, bisection is run for a fixed 26 iterations. At each probe,
$L_{\mathrm{mid}} = \tfrac{1}{2}(L_{\mathrm{low}} + L_{\mathrm{high}})$ is tested with SA; feasible
updates $L_{\mathrm{high}} \leftarrow L_{\mathrm{mid}}$, infeasible updates
$L_{\mathrm{low}} \leftarrow L_{\mathrm{mid}}$.

To reduce restart cost between consecutive probes, we use warm starts.
When a previous feasible state exists at $L_{\mathrm{old}}$, polygon centers are scaled by
$\alpha = L_{\mathrm{new}}/L_{\mathrm{old}}$ before the next simulated annealing run, preserving layout structure
and reducing the time needed to recover feasibility.
...
\begin{enumerate}[leftmargin=1.2em]
  ...
  \item For $k=1,\dots,26$:
    \begin{enumerate}[leftmargin=1.4em]
      ...
      \item Run simulated annealing feasibility probe at $L_{\mathrm{mid}}$.
      \item If feasible:
        $L_{\mathrm{high}} \leftarrow L_{\mathrm{mid}}$,
        save warm state,
        update best feasible $L^*$.
      \item Else:
        $L_{\mathrm{low}} \leftarrow L_{\mathrm{mid}}$.
    \end{enumerate}
\end{enumerate}
...
probes to maintain a target success rate of roughly 35\%, then runs simulated annealing
...
All computational experiments were conducted on the University of
 Arizona high-performance computing (HPC) cluster.
  The results reported here were executed as single-node SLURM array jobs.
...
then the final recovered packing configurations and their packing densities, and finally a
...
It is possible that no feasible configuration was found within the time budget.
This is the reason we run multiple independent trials per $N$:
 to increase the probability of finding at least one feasible configuration.
If we find multiple feasible configurations across different runs, we report the best one.
\begin{table}[!tbp]
\centering\small
...
budget. The trajectories show rapid early gains followed by extended
plateaus with smaller late improvements. This indicates diminishing
returns in long single runs and supports allocating more budget to many
independent shorter trials, especially at larger $N$.
...
The interesting aspect of our approach was the use of 
...
We observe that many of the final gains in a configuration are achieved during the adaptive polish phase. 
Once the outer
...
to find a better rearrangement, so an adaptive update rule for the penalty budget is needed.
A much smarter and more effective way to reduce this bottleneck is to launch many short,
...
Future work must utilize this lesson. First, estimate the average
time needed to obtain a sound packing, then within that budget run as many
independent trials as possible and select the best ones
